"""Redding Planning Commission — Meetings (Granicus archive + HLS manifest)."""
...
class ReddingPlanningCommissionMeetingsSpider(SidekickSpider):
    """Redding Planning Commission — Meetings."""

    name = "redding-planning-commission-meetings"
    source_id = "src_planning_commission_us_ca_shasta_redding_meetings"
    endpoint = "https://reddingca.granicus.com/ViewPublisher.php?view_id=4"
    beat = BeatIdentifier("government:planning-commission")
    ...
    def parse(self, response):
        panels = response.xpath(
            ...
            "[.//div[contains(@class,'CollapsiblePanelTab')]"
            "[contains(normalize-space(.), 'Redding Planning Commission')]]"
        )
