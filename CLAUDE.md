...
│       └── src/sidekick/
│           ├── core/           # models, artifact_store, event_bus, object_store, agent_config
│           └── registry/       # SourceRegistry (used by ingestion + api)
├── services/
│   ├── ingestion/              # examination + ingestion worker agents
│   ├── processing/             # document / transcript / structured (Phase 3)
...
- Agents communicate through the artifact store and event bus only — never import one agent module from another.
...
- Never import `LocalEventBus` or `AWSEventBus` directly in agent code — always inject `EventBus` and `ObjectStore` as dependencies.
...
- Prefer `Protocol` over `ABC` for interfaces (`core/event_bus.py` and `core/object_store.py` are the reference)
